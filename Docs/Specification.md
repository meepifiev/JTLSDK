...
Только в Play Mode и только в редакторе. В полосе инструментов вкладки Game, слева от «Play Focused», стоит кнопка `JTL · RU · Mobile ▾`. Картинку игры она не закрывает. По нажатию под ней открывается панель, клик мимо закрывает её:
...
Смена языка вызывает `Language.Changed`. Переключатель звука меняет `Audio.IsPlatformMuted` и есть только на конфигурациях с `Capability.PlatformMute`. Если окно Game слишком узкое, кнопка встаёт под полосу инструментов в правый угол.
