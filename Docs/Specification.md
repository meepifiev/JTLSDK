...
- `bridge.yml`: на каждый push и PR собрать `Bridge~` (Node 20, `tsc`) и сравнить результат с мостами в репозитории.
- `release.yml`: на тег `v*` проверить, что `package.json` совпадает с тегом, создать GitHub Release с текстом из `CHANGELOG.md`.
- Unity в CI не запускается: на лицензии Personal файл `.ulf` для GameCI больше не получить. EditMode-тесты гоняются в редакторе перед каждым коммитом и обязательно перед тегом релиза.
...
| 22.09.2026 | Workflow `Tests` удалён: на Unity Personal нельзя получить `.ulf` для GameCI, шаг с Unity всегда пропускался, и зелёная галочка ничего не значила. Тесты гоняются в редакторе, перед релизом обязательно. |
| 22.09.2026 | В меню тулкита одна группа MODULES с двумя подкатегориями: Configurable / «Настраиваемые» (покупки, сохранения, языки, лидерборды, флаги) и Ready to use / «Готовые к работе» (реклама, игровые события, пауза, время, звук, игрок, площадка, отзыв, ярлык игры). Отдельной группы Features больше нет. |
